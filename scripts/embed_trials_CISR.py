"""
    Iterate through each trial and get a CISR embedding for each call in the trial
    CISR model checkpoint: https://huggingface.co/AnnaWegmann/Style-Embedding
...
def embed_trials(trials_file, model, embed_outfile): 
    """
    Embed each trial in the trials file using the CISR model
    """
    embeddings = [] 
...
def embed_utterances(speaker_utterances, model):
    """ 
    Get pointwise average of CISR embeddings for each speaker's utterances in a call
    """
...
def main(cfg):
    ...
    if not os.path.exists('CISR_model'):
        os.makedirs('CISR_model')
    
    ### Load the model
    CISR_model = SentenceTransformer('AnnaWegmann/Style-Embedding')
    CISR_model.eval()
        ...
        for dataset in datasets: # 'train', 'val', 'test' 
            for difficulty in difficulties: # 'base', 'hard', 'harder'
                ...
                embed_outfile = os.path.join(work_dir, 
                                        f'CISR_model/{encoding}_{dataset}_{difficulty}_trials.npy')
