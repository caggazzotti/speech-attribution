...
def embed_utterances(speaker_utterances, tokenizer, model):
	...
	"""
	batch_size = 1 #because embedding 1 speaker at a time
	tokenized_text = tokenizer(
		speaker_utterances, 
		max_length=512, #matches original LUAR_encoder setting
		padding="max_length", 
		truncation=True,
		return_tensors="pt"
	)
	episode_length = len(speaker_utterances) # speaker_utterances is one episode 
	tokenized_text = tokenized_text.to("cuda") #need model sent to device AND inputs sent to device
	...
	tokenized_text["attention_mask"] = tokenized_text["attention_mask"].reshape(batch_size,
																				episode_length, -1)
	with torch.inference_mode(): #faster and better performance than no_grad (as long don't need to do training)   
		embed_tensor = model(**tokenized_text)
	embedding = torch.nn.functional.normalize(embed_tensor, p=2, dim=-1) 
	embedding = embed_tensor.cpu().numpy() #convert back to cpu
	return embedding
